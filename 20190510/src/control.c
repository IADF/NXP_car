...
#define parameter_mode    8//0为测试1为参数//参数3，4开始为控制周期6ms的//参数5四元数解算参数//8速度较慢稳定//9速度中等稳定性比较稳，偶尔会失控//10速度较快，不稳定（在调整）
...
float   middle_speed_old_val,middle_speed_val,middle_encoder_old_val,middle_encoder_val;//车模速度，通过左右车轮的线速度均值得到
...
#define angle_up_limit    -54.8   //-52.3

//PWM限制//转速限制
//转速过快容易导致主控电压不足
#define PWM_limit   800 //给1000时为100%PWM输出，当电机全速输出时电机分压过大容易导致主控电压不足

#if  parameter_mode==8//速度环用PI控制，比较稳定，速度180，实际稳定150左右
//直立PID参数
//角速度环PD参数整定
#define static_angle_speed_P   3.2//6.8//34//58//102//64
#define static_angle_speed_I   0.12
...
#define static_angle_P  0.098//0.064//0.098//0.143//-48//-12//42//78//88//108//112//122//142
float balance_angle=-30.4;//34.8;//34.4//-20.7;//-37.4;//机械零点角度 44.8~47//39//35-38大约36//35.4//30.4平衡角度//前进时角度增加
#define angle_output_limit 1100

//速度环通过PI控制//未加加速限幅加速不稳定，容易过冲//纯P控制（容易过冲震荡）（不够稳定）(速度晃动较大)
#define speed_P  3.4//-0.98//171//4.8//32.4//-48.6
...
int speed_val_I=150;
int speed_val_P=150;
#define speed_target_val 150//目标速度


float middle_line_P=158;//20.0;//方向环P参数
float middle_line_I=0;
float middle_line_D=0;//方向环D参数 

int middle_line_PWM_limit=1500;
int direction_output_limit=180;//内环限幅参数
...
//速度环通过PI控制//未加加速限幅加速不稳定，容易过冲//纯P控制（容易过冲震荡）（不够稳定）(速度晃动较大)
#define speed_P  3.86//-0.98//171//4.8//32.4//-48.6
#define speed_I  0.326//0.24
#define speed_D  0//76
...
//速度环通过PI控制//未加加速限幅加速不稳定，容易过冲//纯P控制（容易过冲震荡）（不够稳定）(速度晃动较大)
#define speed_P  3.94//-0.98//171//4.8//32.4//-48.6
#define speed_I  0.326//0.24
#define speed_D  0//76
...
int speed_val_I=220;
int speed_val_P=220;
#define speed_target_val 220//目标速度
...
//       angle_y_now=atan2(mpu_acc_x,mpu_acc_z)*(float)180.0/3.1415926;//加速度通过三角函数解算
       ...
       if(R_curve_flag||L_curve_flag)//急弯检测（改变机械零点）转向时会导致车模后仰（通过改变机械零点抑制后仰）（同时抑制转弯减速）//R_curve_flag||L_curve_flag((middle_Line-theory_middle_line)>8||(middle_Line-theory_middle_line)<-8
       {
         balance_angle=-32.4;
       }
       else 
    ...
    if((ms_task%2)==0)//4ms中线偏差方向环（外环）
    {
      ...
      middle_line_parameter();//更具当前速度更改转向环参数
...
//      middle_line_output_old_val=(int)((float)middle_line_output_old_val*0.7+(float)middle_line_output_val*0.3);
    ...
    direction_output_val=(int)(direction_P*(direction_gyro_err)+direction_D*direcion_gyro_last_err);//串级PID中线量为输入量无需平滑输出
    
    direction_output_last_val=(int)(0.3*direction_output_last_val+0.7*direction_output_val);//转向上升限幅
    ...
    {
        ms_task=0;//计数归零
        if(run_flag==0&&first_cycle_flag==0)//第一个周期由于卡尔曼滤波需要积分周期，所以第一个周期电机不运行
        {
          first_cycle_flag=1;
          run_flag=1;
        }
...
//对编码器进行处理
//当电机疯转时，将速度环关闭等待车稳定
//当电机疯转时开始计数，当电机持续疯转一段时间关闭电机，防止车模撞坏//未完成
void speed_measure(void)////未完成
{
  ...
  if(right_motor_crazy_flag>0)//右电机速度控制
  {
  ...
  if(left_motor_crazy_flag>0)//左电机速度控制
  {
...
void middle_line_parameter(void)
{
  ...
  {
      middle_line_P=124;//20.0;//方向环P参数
      middle_line_I=0;
      middle_line_D=-5;//方向环D参数 

      middle_line_PWM_limit=1500;
      direction_output_limit=120;//内环限幅参数
  ...
  {
      middle_line_P=168;//20.0;//方向环P参数
      middle_line_I=0;
      middle_line_D=-6;//方向环D参数 

      middle_line_PWM_limit=1500;
      direction_output_limit=160;//内环限幅参数
  ...
  {
       middle_line_P=232;//20.0;//方向环P参数
       middle_line_I=0;
       middle_line_D=-8;//方向环D参数 

       middle_line_PWM_limit=1500;
       direction_output_limit=200;//内环限幅参数
  ...
  {
       middle_line_P=256;//20.0;//方向环P参数
       middle_line_I=0;
       middle_line_D=-8;//方向环D参数 

       middle_line_PWM_limit=1500;
       direction_output_limit=220;//内环限幅参数
