...
#define parameter_mode 6 //0为测试1为参数//参数3，4开始为控制周期6ms的//参数5四元数解算参数
...
//直立PID参数
//角速度环PD参数整定
#define static_angle_speed_P  30//34//58//102//64
#define static_angle_speed_I   0
#define static_angle_speed_D  -0//-12
...
#define right_deal_val  0
//#define static_down_angle_P  42
#define static_angle_P  -0//-48//-12//42//78//88//108//112//122//142
float balance_angle=-34.8;//34.8;//34.4//-20.7;//-37.4;//机械零点角度 44.8~47//39//35-38大约36//35.4
#define angle_output_limit 0
...
void PIT0_IRQHandler(void)//2ms角度环
{
    ...
    if((ms_task%20)==0)//40ms速度环
    { 
      middle_speed_val=middle_encoder_old_val*0.7+0.3*middle_encoder_val;
      ...
      speed_output_val=speed_P*(middle_speed_val+speed_val_P)+speed_I*(speed_integral_val+speed_val_I)+speed_D*(middle_speed_val-middle_encoder_old_val);//PID
      
      if(speed_output_val>speed_output_limit) speed_output_val=speed_output_limit;//速度环输出限幅
      else if(speed_output_val<-speed_output_limit) speed_output_val=-speed_out_limit;
      
      OLED_Print_Num1(84,0,middle_encoder_val);
      
      middle_encoder_old_val=middle_speed_val;
      middle_encoder_val=0;
    }
    if((ms_task%3)==0)//6ms角度环
    {
       ...
       angle_output_val=static_angle_P*(10*(angle-balance_angle)-speed_smoothness_output())+static_angle_D*((10*(angle-balance_angle)-speed_smoothness_output())-angle_last);
       angle_last=10*(angle-balance_angle)-speed_smoothness_output();
...
//   
//      //比例限制方向PWM增长 (使转向圆滑)        
//      direction_output_old_val=(int)((float)direction_output_old_val*0.7+(float)direction_output_val*0.3);
//    
//      //方向环限幅
...
//         direction_output_val=-direction_PWM_limit;
    ...
    motor_right_val=motor_val+direction_smoothness_output();
    motor_left_val=motor_val-direction_smoothness_output();
...
int speed_smoothness_output()
{
  ...
  speed_smoothness_output_val=speed_smoothness_output_old_val+(int)((float)(speed_output_val-speed_smoothness_output_old_val)*(float)speed_count/20.0);//速度环周期为4ms//每2毫秒进行一次电机控制
...
void mpu_normalization_point()//归一化
{
  ...
  mpu_acc_float_x=(float)mpu_acc_x*0.2392615/3.0;
  mpu_acc_float_y=(float)mpu_acc_y*0.2392615/3.0;
  mpu_acc_float_z=(float)mpu_acc_z*0.2392615/3.0;
