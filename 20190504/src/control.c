...
#define parameter_mode 9 //0为测试1为参数//参数3，4开始为控制周期6ms的//参数5四元数解算参数
...
//速度环相关变量
int left_encoder_val,right_encoder_val,left_encoder_old_val,right_encoder_old_val,middle_encoder_val; //编码器数值
...
//速度环通过PI控制
#define speed_P  0.4//171//4.8//32.4//-48.6
#define speed_I  0.02
#define speed_D  -0//76
#define speed_destination 0//车模期望速度-为电池方向
#define speed_curve_destination 0//车模转弯的期望速度
...
//    speed_measure();
    ...
    {         
        OLED_Print_Num1(76,0,left_encoder_val);
...
//       angle_y_now=atan2(mpu_acc_x,mpu_acc_z)*(float)180.0/3.1415926;//加速度通过三角函数解算
       ...
       angle_output_val=static_angle_P*(100*(angle-balance_angle)-speed_smoothness_output())+static_angle_D*((100*(angle-balance_angle)-speed_smoothness_output())-angle_last);
       angle_last=100*(angle-balance_angle)-speed_smoothness_output();
...
//      middle_line_output_old_val=(int)((float)middle_line_output_old_val*0.7+(float)middle_line_output_val*0.3);
    ...
    //2ms方向角速度环（内环）
    direction_output_val=(int)(direction_P*(mpu_gyro_float_z*10-direction_smoothness_output()));
...
void speed_measure(void)//对编码器进行处理
{
  //左编码器处理
  if(left_encoder_val>speed_target_val+50)//疯转
  {
    left_encoder_old_val=(int)(0.9*left_encoder_val+0.1*left_encoder_old_val);
  }
  ...
  {
    left_encoder_old_val=(int)(0.5*left_encoder_val+0.5*right_encoder_old_val);
  }
  else//正常输出
  {
    left_encoder_old_val=(int)(0.9*left_encoder_val+0.1*left_encoder_old_val);
  ...
  {
    right_encoder_old_val=(int)(0.5*right_encoder_val+0.5*right_encoder_old_val);
  }
  ...
  {
    right_encoder_old_val=(int)(0.9*right_encoder_val+0.1*right_encoder_old_val);
  }
  
  middle_encoder_val=(right_encoder_old_val+left_encoder_old_val)/2;
  middle_encoder_old_val=(int)(0.8*middle_encoder_val+0.2*middle_encoder_old_val);
